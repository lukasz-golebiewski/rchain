...
object Substitute {
  private[interpreter] def charge[A: Chargeable, M[_]: _cost: _error](
    ...
    substitutionResult.attempt
      ...
      .flatMap({ case (result, cost) => accounting.charge[M](cost) *> _error.pure(result) })
      .rethrow

  def substituteAndCharge[A: Chargeable, M[_]: _cost: _error: Substitute[?[_], A]](
      term: A,
      depth: Int,
      env: Env[Par]
  ): M[A] =
  ...
  def substituteNoSortAndCharge[A: Chargeable, M[_]: _cost: _error: Substitute[?[_], A]](
      term: A,
      depth: Int,
      env: Env[Par]
  ): M[A] =
